...
// remove old data
l = this.settings.swingperiod
if (l < this.settings.period) l = this.settings.period

if (this.history.close.length > l) {
  this.history.close.shift();
  this.history.open.shift()
  this.history.low.shift();
  this.history.high.shift()
...
var calculateSMA = function(candle) {
  ...
  if (this.swing.buycount == 3) {
    ...
    this.swing.tradebuy = false
  }
  if (this.swing.sellcount == 3) {
    this.swing.sellcount = 0
    this.swing.tradesell = true
  } else {
    this.swing.tradesell = false
  }
}.bind(this)
...
this.calculateVIX(candle, this.settings.vixperiod)
  .then(this.gannswing(candle, this.settings.swingperiod))
}
...
if (this.trend.adviced && this.trend.direction === 'long') {
  sl = this.history.buyPrice * (1 - this.settings.stoploss.percent / 100)
  if (price < sl) {
  ...
  if (this.settings.stoploss.trailing && price > this.history.buyPrice) {
    this.history.buyPrice = price
    return false
  }
}
